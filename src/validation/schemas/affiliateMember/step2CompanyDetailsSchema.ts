...
/**
 * Step 2 Company Details validation schema - Affiliate Member
 * Validates company info, shareholders, UBOs, directors with dynamic arrays
 * Similar structure to Principal Member
 */
...
export const step2CompanyDetailsSchema = Yup.object({
  ...
  // Documents - Trade License & COI - Accept File OR existing document ID
  tradeLicenseFile: Yup.mixed()
    .nullable()
    .test('file-required', 'Trade license document is required', function(value) {
      const { tradeLicenseFileId } = this.parent;
      console.log('🔍 Validating tradeLicenseFile:', {
        file: value instanceof File ? 'File present' : 'No file',
        documentId: tradeLicenseFileId,
        isValid: !!(value instanceof File || tradeLicenseFileId)
      });
      // Valid if there's a new file OR an existing document ID
      if (value instanceof File) return true;
      if (tradeLicenseFileId) return true;
      return false;
    }),
  ...
  coiFile: Yup.mixed()
    .nullable()
    .test('file-required', 'Certificate of incorporation is required', function(value) {
      const { coiFileId } = this.parent;
      console.log('🔍 Validating coiFile:', {
        file: value instanceof File ? 'File present' : 'No file',
        documentId: coiFileId,
        isValid: !!(value instanceof File || coiFileId)
      });
      // Valid if there's a new file OR an existing document ID
      if (value instanceof File) return true;
      if (coiFileId) return true;
      return false;
    }),
  ...
  anyShareholderDirectorUBOPEP: Yup.boolean()
    ...
    .test(
      'is-selected',
      'Please answer if any shareholder/director/UBO is a PEP',
  ...
  anyShareholderBeneficialOwnerKeyPersonRelatedToPEP: Yup.boolean()
    ...
    .test(
      'is-selected',
      'Please answer if any shareholder/beneficial owner/key person is related to a PEP',
      (value) => value !== null && value !== undefined
    ),
  hasCustomerPEPChecks: Yup.boolean()
    ...
    .test(
      'is-selected',
      'Please answer if your establishment checks for PEP customers',
      (value) => value !== null && value !== undefined
    ),

  // Trade Association
  tradeAssociationName: requiredString('Trade association name'),
  nameOfMember: requiredString('Name of member'),
  dateOfAppointment: requiredString('Date of appointment'),
  ...
  tradeAssociationCertificateFile: Yup.mixed()
    .nullable()
    .test('file-required', 'Trade association certificate is required', function(value) {
      const { tradeAssociationCertificateFileId } = this.parent;
      console.log('🔍 Validating tradeAssociationCertificateFile:', {
        file: value instanceof File ? 'File present' : 'No file',
        documentId: tradeAssociationCertificateFileId,
        isValid: !!(value instanceof File || tradeAssociationCertificateFileId)
      });
      if (value instanceof File) return true;
      if (tradeAssociationCertificateFileId) return true;
      return false;
    }),
